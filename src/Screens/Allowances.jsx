...
class Allowances extends Component {
  ...
  async loadAllowances(){
      const {emp_id, month, year} = this.props.match.params;
      var fetchData = await fetch(API_URL + ADMIN_ALLOWANCE_LIST + `${emp_id}/${month}/${year}`).then(res => res.json());
      if(fetchData.code === 200){
          this.setState({
                 ...
                 <table className="table table-striped text-center table-bordered">
                   ...
                   <tbody>
                     {allowances.map((item,index) => {
                       return(
                         <tr key={index}>
                            ...
                            <td>
                              <span className="badge badge-info p-2">{moment(item.created_at).format('D MMM Y')}</span>
                            </td>
                            <td>
                              {item.status === 2 ?
                                <span className="badge badge-info p-2">{moment(item.al_date).format('D MMM Y')}</span>
                                :
                                <span className="badge badge-info p-2">NA</span>
                              }
                            </td>
                            <td>
                              <select className="rounded p-2" value={item.status} onChange={(e) => this.changeStatus(e,item.id)}>
                     ...
                     <tr>
                       ...
                       <td>
                          <span className="badge badge-info p-2">${tot_bonus}</span>
                        </td>
                       <td>
                           <span className="badge badge-info p-2">${tot_per}</span>
                      </td>
                       <td colSpan="3" ></td>
                     </tr>
                     <tr>
                       <td colSpan="5" >
                         <strong className="text-danger font-bold float-right">Total</strong>
                       </td>
                       <td colSpan="2">
                          <span className="badge badge-info p-2">${parseFloat(tot_bonus) + parseFloat(tot_per)}</span>
                        </td>
                  ...
                  <div className="row">
                    <div className="col-md-6">
                      <div className="form-group">
                        ...
                        <select className="form-control" value={ this.props.match.params.emp_id} onChange={this.changeName} disabled>
                ...
                <form>
                    <div className="row">
                      <div className="col-md-6">
                        <div className="form-group">
                          <label>Person Name</label>
                          <select className="form-control" value={editname} onChange={this.changeEditName} disabled>
                          <option value="">Select one</option>
                        {
                          employees !== null &&
                          employees.map((item,index) => {
