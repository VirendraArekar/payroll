...
import {
    ...
    API_URL,
    EMPLOYEE_EXSPENSES,
    EMPLOYEE_EXSPENSE_UPDATE,
    EMPLOYEE_EXSPENSE_EDIT,
    EMPLOYEE_EXSPENSE_CREATE,
    EMPLOYEE_EXSPENSE_DELETE,
    EMPLOYEE_EXSPENSE_SEARCH,
    ADMIN_EXSPENSE_STATUS_CHANGE
} from '../../Contants';
import moment from 'moment';

class EmpExpenses extends Component {
  constructor(props) {
    super(props);
    this.state = {
       item : '',
  ...
  async editAddId(id){
      ...
      var fetchData = await fetch(API_URL + EMPLOYEE_EXSPENSE_EDIT + id).then(res => res.json());
      if(fetchData.code === 200){
        this.setState({
            ...
            editpurchase_by : this.state.userId,
  ...
  async loadExspenses(){
      ...
      var fetchData = await fetch(API_URL+EMPLOYEE_EXSPENSES+user.id).then(res => res.json());
      if(fetchData.code === 200){
          this.setState({
  ...
  async AddExpense(e){
      e.preventDefault()
      const  {userId,item,purchase_from,amount,purchase_by,paid_by,status} = this.state;
      var error = false;
      ...
      //   formData.append('file_', image_file);
      // });

      // for(var i = 0; i < this.state.images.length ; i++){
      //   formData.append('file_'+i, this.state.images[i])
      // }

      formData.append('photo', this.state.photo);

	  	var fetchData = await fetch(
			API_URL+ EMPLOYEE_EXSPENSE_CREATE,
			{
				method: 'POST',
				body: formData,
			}
     ...
     var fetchData = await  fetch(API_URL + EMPLOYEE_EXSPENSE_CREATE,{
         method : 'post',
  ...
  async UpdateExspense(e){
      e.preventDefault();
      const  {userId,edititem,editpurchase_from,editamount,editpurchase_by,editpaid_by,editstatus} = this.state;
      var error = false;
      ...
      formData.append('photo', this.state.images);
	  	...
	  	var fetchData = await fetch(API_URL+ EMPLOYEE_EXSPENSE_UPDATE,
                        {
                          method: 'POST',
                          body: formData,
                        }
  ...
  async deleteExspense(e){
      e.preventDefault();
      var fetchData = await fetch(API_URL + EMPLOYEE_EXSPENSE_DELETE + this.state.deleteId).then(res => res.json());
  ...
  async searchExspense(e){
    ...
    var fetchData = await  fetch(API_URL + EMPLOYEE_EXSPENSE_SEARCH,{
        method : 'post',
        headers:{'content-type': 'application/json'},
        body : JSON.stringify(data)
    }).then(res=>res.json());
      ...
      },
      {
        ...
        render: (text, record) => (
            <h2 className="table-avatar">
             <img alt="" src={record.emp.photo ?  `${CORE}uploads/photo/${record.emp.photo}` : 'https://via.placeholder.com/150'} style={{width : 40}} className="circle"/>
            ...
            <div className="dropdown dropdown-action text-right">
                  <a href="#" className="action-icon dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i className="material-icons">more_vert</i></a>
                      <div className="dropdown-menu dropdown-menu-right">
                        ...
                        <a className="dropdown-item" href="#" data-toggle="modal" data-target="#delete_expense" onClick={() => this.editAddDeleteId(record.id)}><i className="fa fa-trash-o m-r-5" /> Delete</a>
                      </div>
            </div>
          ),
      },
                ...
                <form>
                  ...
                  <div className="row">
                    ...
                    <div className="col-md-6">
                      <div className="form-group">
                        ...
                        <input className="form-control" type="file" onChange={this.clickImage} accept="image/x-png,image/gif,image/jpeg" />
                      </div>
                    </div>
                  </div>
                  <div className="attach-files">
                    <ul>
                      ...
                      {/* {
                        this.state.imgView.map((item,index) => {
                          return(
                            <li key={index}>
                              <img src={item} alt="" />
                              <a href="#" className="fa fa-close file-remove" onClick={() => this.removeImg(index)}/>
                            </li>
                          )
                        })
                      } */}
                ...
                <form>
                  ...
                  <div className="row">
                    <div className="col-md-6">
                      <div className="form-group">
                        <label>Purchase Date <span className="text-danger">*</span></label>
                        <div className="cal-icon"><input className="form-control"  id="pdatess" type="text" value={editpurchase_date} onChange={this.changeEditPurchaseDate}/></div>
                        {
                            editpurchaseDateError &&
                  ...
                  <div className="row">
                    ...
                    <div className="col-md-6">
                      <div className="form-group">
                        ...
                        onChange={this.clickEditImage} accept="image/x-png,image/gif,image/jpeg" />
                      </div>
                    </div>
                  </div>
                  <div className="attach-files">
