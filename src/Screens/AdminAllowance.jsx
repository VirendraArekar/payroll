...
import {
    ...
    ADMIN_ALLOWANCE_LIST_SEARCH
} from '../Contants';
import moment from 'moment';
import Operation from 'antd/lib/transfer/operation';

class AdminAllowance extends Component {
  constructor(props) {
    super(props);
    this.state = {
      name : '',
  ...
  async loadAllowances(){
      var fetchData = await fetch(API_URL + ADMIN_ALLOWANCES).then(res => res.json());
      if(fetchData.code === 200){
          this.setState({
              allowances : fetchData.data['allowances'],
              clients : fetchData.data['clients'],
              employees : fetchData.data['employees']
            })
      }
  }
  ...
  async searchAllowance(e){
    ...
    var fetchData = await  fetch(API_URL + ADMIN_ALLOWANCE_LIST_SEARCH,{
        method : 'post',
        headers:{'content-type': 'application/json'},
        body : JSON.stringify(data)
    }).then(res=>res.json());
   ...
   render() {
    ...
    const columns = [

      {
        ...
        dataIndex: 'emp_name',
        render: (text, record) => (
            <a href={`/admin/show/allowances/${record.month}/${record.year}/${record.emp_id}`} style={{color : 'grey'}}>
                <img src={record.post !== '' || record.post !==  undefined ? CORE+`uploads/photo/${record.photo}` : Avatar_02} style={{width : 40}} className="circle"/>
                {record.emp_name} - &nbsp;
                <span className="badge badge-secondary p-1">{record.post}</span>
            </a>
      ...
      //   dataIndex: 'bonus',
      //   render: (text, record) => (
      //       <span className="badge badge-secondary p-2">${record.bonus}</span>
      //   ),
      ...
      //   dataIndex: 'per_diem',
      //   render: (text, record) => (
      //   <span className="badge badge-secondary p-2">${record.per_diem}</span>
      //     ),
      ...
      // },
      {
        ...
        // dataIndex: 'per_diem',
        render: (text, record) => (
            <span className="badge badge-secondary p-2">${parseFloat(record.per_diem) + parseFloat(record.bonus)}</span>
          ),
      },
      {
        title: 'Month Year',
        dataIndex: '',
        render: (text, record) => (
          <span className="badge badge-info p-2">{moment(`${record.year}-${record.month}-01`).format('MMM, Y')}</span>
        ),
      ...
      },
      {
        ...
        dataIndex: '',
        render: (text, record) => (
          <span className="badge badge-info p-2">{record.count}&nbsp;Days</span>
        ),
        // sorter: (a, b) => a.al_date.length - b.al_date.length,
      },
    //   {
    ...
    //   },
      {
        ...
        dataIndex: '',
        render: (text, record) => (
            <a className="btn btn-custom" href={`/admin/show/allowances/${record.month}/${record.year}/${record.emp_id}`}><span className="fa fa-eye"></span>&nbsp;view</a>
          ),
      },
    //   {
    //     title: 'Action',
                  ...
                  <div className="row">
                    <div className="col-md-6">
                      <div className="form-group">
                        ...
                        <select value={name} onChange={this.changeName}  className="form-control">
